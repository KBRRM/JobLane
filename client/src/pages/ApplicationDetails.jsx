...
import { getSingleApplication, updateApplicationStatus } from '../actions/ApplicationActions';
...
export const ApplicationDetails = () => {
    ...
    const [status, setStatus] = useState(applicationDetails.status || "pending");
                        ...
                        <div>
                            ...
                            <div className='pt-4 pb-6'>
                                ...
                                <div>
                                    <ul>
                                        ...
                                        <li className='flex gap-4 items-center'>Resume: <Link to={applicationDetails.applicantResume.url} target="_blank"
                            ...
                            <div className='pt-2 pb-2'>
                                <div className='flex gap-3 items-center text-xl'>
                                    Status: <span className={`${applicationDetails.status === "pending" ? "text-blue-600" :
                                        applicationDetails.status === "rejected" ? "text-red-600" : "text-green-600"
                                    } font-medium`}>{toUpperFirst(applicationDetails.status)}</span>
                                </div>
                            </div>

                            {/* New Feature: Update Status Dropdown */}
                            <div className='pt-2 pb-4'>
                                <div className='flex gap-3 items-center text-xl'>
                                    ...
                                    <button onClick={updateStatusHandler} className="ml-3 bg-green-600 hover:bg-green-800 py-2 px-4 text-white rounded">
                                        Update Status
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className='pt-2 pb-2'>
                                <div className='flex gap-3 items-center text-xl'>
                                    Application Created At: {convertDateFormat(applicationDetails.createdAt.substr(0, 10))} on {extractTime(applicationDetails.createdAt)}
                                </div>
                            </div>
